...
class ControllerCatalogForm extends Controller {
	...
	public function index() {
		$this->language->load('catalog/form');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/form', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('catalog/form/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('catalog/form/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$results = $this->model_catalog_form->getforms($filter_data);
		...
		foreach ($results as $result) {
			$data['forms'][] = array(
				...
				'edit'        => $this->url->link('catalog/form/edit', 'token=' . $this->session->data['token'] . '&form_id=' . $result['form_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/form', 'token=' . $this->session->data['token'] . '&sort=dd.name' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('catalog/form', 'token=' . $this->session->data['token'] . '&sort=d.date_added' . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/form_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['form_id'])) {
			$data['action'] = $this->url->link('catalog/form/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('catalog/form/edit', 'token=' . $this->session->data['token'] . '&form_id=' . $this->request->get['form_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('catalog/form', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['form_description'])) {
			...
			$data['form_description'] = $this->model_catalog_form->getformDescriptions($this->request->get['form_id']);
		} else {
		...
		if (isset($this->request->post['filename'])) {
			...
			$data['filename'] = $form_info['filename'];
		} else {
			$data['filename'] = '';
		}
		...
		$this->response->setOutput($this->load->view('catalog/form_form.tpl', $data));
	}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $form_id) {
			$news_total = $this->model_catalog_news->getTotalNewssByformId($form_id);
